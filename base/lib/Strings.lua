...
--[[
	for StartIndex, EndIndex, CaptureSubString in Strings.FindItterator("aba", "a") do
		print(StartIndex, EndIndex, CaptureSubString)
	end
	--> 1 1 a
		3 3 a
]]
function Strings.FindItterator(str, find)
	return
...
function Strings.BuildString(Char, Amount)
	return string.rep(Char, Amount)
end

function Strings.NthCharInString(str, char, n)
...
-- EX: print(Strings.FormatTime("%H2:%M2:%S2", 340, 3, 1)) -> 05:40
function Strings.FormatTime(format, seconds, start, clip)
	local hitNonZero = false
	start = start or #Strings.FORMATS
	
	seconds = math.floor(seconds)
	for i = start, 1, -1 do
		local pat = Strings.FORMATS[i]

		local s, e = string.find(format, pat, 1, true)
		if s then
			local name = formatsToName[pat]
			local value = math.floor(seconds / Strings.SECONDS_IN_A_[name])
			local secs = value * Strings.SECONDS_IN_A_[name]
			...
			if fill then
				...
				if num then
					e += 1
					value = Strings.BuildString("0", num - #value) .. value
				end
			end
...
function Strings.NumToAlphabetOrder(int)
...
function Strings.NthFind(Search_str, find_str, n, plain)
	local finds = {}
...
function Strings.Extract(output_code)
