...
local function tests_wrapper(module, name)
	...
	-- Configure the builder for this module
	local builder = Tests.Tester( name )
...
function mod.__raw_load(script: Instance, name: string): any
	...
	add_module(CollectedModules, name, module)
	add_module(PreLoads, name, module)
	Initialized[name] = false

	return module
end

function mod.PreLoad(script: Instance, opt_name: string?)
...
local function recursive_collect(instance: Folder | ModuleScript)
...
type r = typeof(mod.CollectModules(mod))
